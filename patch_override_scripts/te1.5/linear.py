...
class _Linear(torch.autograd.Function):
    ...
    def forward(
        ...
        if fp8:
            ...
            if FW_EMULATE_FP8 > 0:
                with emulate_fp8():
                    im_tmp = cast_from_fp8(inputmat_total,
                                           fp8_meta['scaling_fwd'],
                                           tex.FP8FwdTensors.GEMM1_INPUT,
                                           fp8_dtype_forward,
                                           TE_DType[torch.bfloat16])
                    w_tmp = cast_from_fp8(weight_fp8._data,
                                          fp8_meta["scaling_fwd"],
                                          tex.FP8FwdTensors.GEMM1_WEIGHT,
                                          fp8_dtype_forward,
                                          TE_DType[torch.bfloat16])
            ...
            if ctx.requires_dgrad:
                if ctx.fp8:
                    if BW_USE_MXFP4 == 1 or BW_USE_MXFP4 == 3:
                        S = (had.T * ((torch.randn(HBS, device=weight.device) > 0).to(had.dtype) * 2 - 1)).T
                        dgrad = mx.matmul(
                        ...
                        dgrad = mx.matmul(grad_output, weight, mx_specs=mxfp4)
                    else:
                        dgrad, _ = fp8_gemm(
                            weight_t_fp8._data,
                            fwd_scale_inverses,
            ...
            if weight.requires_grad:
                if ctx.fp8:
                    ...
                    if not ctx.fp8_meta["recipe"].override_linear_precision.wgrad:
                        ...
                        if BW_USE_MXFP4 == 1 or BW_USE_MXFP4 == 3:
                            im_tmp = tex.cast_from_fp8(inputmat_t_total.reshape(1, -1),
                                                       fwd_scale_inverses, fp8_dtype_forward,
                                                       TE_DType[had.dtype]).reshape(inputmat_t_total.shape).T
                            S = (had.T * ((torch.randn(HBS, device=weight.device) > 0).to(had.dtype) * 2 - 1)).T
                            wgrad = mx.matmul(
                                (grad_output.T.reshape(-1, HBS) @ S).reshape(grad_output.T.shape),
                                (im_tmp.T.reshape(-1, HBS) @ S).reshape(im_tmp.T.shape).T,
                                mx_specs=mxfp4)
                        elif BW_USE_MXFP4 == 2 or BW_USE_MXFP4 == 4:
                            im_tmp = tex.cast_from_fp8(inputmat_t_total.reshape(1, -1),
                                                       fwd_scale_inverses, fp8_dtype_forward,
                                                       TE_DType[had.dtype]).reshape(inputmat_t_total.shape).T
                            wgrad = mx.matmul(grad_output.T.contiguous(), im_tmp, mx_specs=mxfp4)
                        else:
                            wgrad, _ = fp8_gemm(
                                inputmat_t_total,
                                fwd_scale_inverses,
