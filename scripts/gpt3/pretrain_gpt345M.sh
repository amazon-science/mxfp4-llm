...
if [ -v SLURM_NNODES ]; then
    ...
    NODEID=$SLURM_NODEID
    NTASKS=$SLURM_NTASKS
    TS_PATH=/workspace/checkpoints/gpt_345m_BW${BW_USE_MXFP4}_FP8FW${FW_USE_FP8}_FP8EMU${FW_EMULATE_FP8}_JOB${SLURM_JOB_ID}_$(date '+%Y%m%d%H%M%S')
else
    # Single-node, non-SLURM runs
    HOSTS=(localhost)
    NODEID=0
    NTASKS=1
    TS_PATH=/workspace/checkpoints/gpt_345m_BW${BW_USE_MXFP4}_FP8FW${FW_USE_FP8}_FP8EMU${FW_EMULATE_FP8}_$(date '+%Y%m%d%H%M%S')
fi
